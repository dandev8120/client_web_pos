...
export function getOrderDetailFull(order: DataType): OrderDetailFull {
  ...
  return {
    ...
    items: itemsList,
    totals: {
      subtotal: `${(totalAmountVal + itemsList.reduce((sum, item) => sum + item.discount, 0)).toLocaleString('vi-VN')} ₫`,
      productDiscountTotal: `${itemsList.reduce((sum, item) => sum + item.discount, 0).toLocaleString('vi-VN')} ₫`,
      orderDiscountTotal: '0 ₫',
      voucherTotal: `${voucherAmountVal.toLocaleString('vi-VN')} ₫`,
      discountTotal: `${itemsList.reduce((sum, item) => sum + item.discount, 0).toLocaleString('vi-VN')} ₫`,
      shippingFee: '0 ₫ (Bán Lẻ tại quầy)',
      surcharge: '0 ₫',
      vatTotal: `${itemsList.reduce((sum, item) => sum + item.vat, 0).toLocaleString('vi-VN')} ₫`,
