...
import { Upload, Button, List, Space, Typography, Image, Card, App, Tooltip, Avatar, Divider } from 'antd';
import { 
  UploadOutlined, 
  DeleteOutlined, 
  EyeOutlined, 
...
const FancyUpload: React.FC<FancyUploadProps> = ({ 
  ...
  const handleUpload = (info: any) => {
    // Collect new files from info.fileList that aren't already represented in our state by UID
    // Actually, simple appending might be better if we handle beforeUpload returning false
    const incomingFiles = info.fileList.map((file: any) => ({
      uid: file.uid,
      name: file.name,
      status: 'done',
    ...
    // Merge with existing fileList, avoiding duplicates by uid
    setFileList(prev => {
      const existingUids = new Set(prev.map(f => f.uid));
      const filteredIncoming = incomingFiles.filter((f: any) => !existingUids.has(f.uid));
      const newList = [...prev, ...filteredIncoming];
      onChange?.(newList);
      return newList;
    });
  };
  ...
  const previewFile = (file: FileItem) => {
    if (file.url) {
      ...
      const isText = file.name.match(/\.(txt|md|js|ts|json|html|css|csv)$/i);
      const isOffice = file.name.match(/\.(doc|docx|xls|xlsx|ppt|pptx)$/i);
      ...
      modal.info({
        ...
        content: (
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            {isImage ? (
              <img src={file.url} alt={file.name} style={{ maxWidth: '100%', maxHeight: '70vh', objectFit: 'contain' }} />
            ) : isPdf ? (
              <iframe src={`${file.url}#toolbar=0`} style={{ width: '100%', height: '70vh', border: 'none' }} title={file.name} />
            ) : isOffice ? (
              <div style={{ padding: 40, background: '#f5f5f5', borderRadius: 8 }}>
                <FileOutlined style={{ fontSize: 64, color: '#1677ff' }} />
                <Title level={4} style={{ marginTop: 16 }}>{file.name}</Title>
                <Text type="secondary">This is an Office document. Browsers cannot preview these directly without a dedicated viewer.</Text>
                <div style={{ marginTop: 24 }}>
                   <Space>
                    <Button type="primary" onClick={() => window.open(file.url)}>Download to View</Button>
                    <Button onClick={() => window.open(`https://view.officeapps.live.com/op/view.aspx?src=${encodeURIComponent(file.url || '')}`)}>View via Office Online</Button>
                   </Space>
                </div>
              </div>
            ) : isText ? (
